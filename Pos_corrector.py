...
with open(curr_path + filename, encoding='cp1251') as f:
    t = f.read().split('\r\n')
    ' '.join(t)
    query = t[0].replace('\n', ' ')

# ПОЗИЦИИ
print('Забираем позиции')
pos_import = parser_utils.select_query(query, login_sql, password_sql)

if pos_import.empty == False:
    i_pos = 1
    for index_pos, row_pos in pos_import.iterrows():
        time.sleep(0.25)
        print(index_pos, '/', len(pos_import))

        prodtype = row_pos['ProdType']
        prodnm = row_pos['ProdNm']
        pos_id = row_pos['LotSpec_ID']
        ...
        print('Name:', prodnm, 'Prodtype:', prodtype)
        ...
        cursor = conn.cursor()
        cursor.execute(
                "update [Cursor].[dbo].[LotSpec] set ProdType = ?, LS_MNN_ID = ?, LS_TradeNm_ID = ?, LotSpecMtdID = ?, UPDDATE=getdate() where LotSpec_ID = ? ",
            prodtype, mnn_pos, tn_pos, mtd_pos, pos_id)
        if isDebug == False:
            conn.commit()
        conn.close()
